...
class RegisterForm(UserCreationForm):
    ...
    consentement_rgpd = forms.BooleanField(
        ...
        error_messages={
            "required": "Vous devez accepter la politique de confidentialité pour créer un compte."
        },
    )
...
class LoginForm(forms.Form):
    ...
    password = forms.CharField(
        label="Mot de passe", widget=forms.PasswordInput(attrs={"aria-required": "true"})
    )
...
class DataExportForm(forms.Form):
    confirm = forms.BooleanField(
        required=True, label="Je confirme vouloir exporter l'ensemble de mes données personnelles."
    )
