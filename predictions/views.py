...
from .forms import PredictionForm, BatchUploadForm, PredictionFilterForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if prediction.status == 'success':
                messages.success(request, f"Prédiction effectuée : {prediction.prix_estime:,.0f} €")
                return redirect('prediction_detail', pk=prediction.pk)
            else:
                messages.error(request, f"Erreur lors de la prédiction : {prediction.error_message}")
    else:
        form = PredictionForm()
...
def api_health(request):
    from .services import api_client, APIClientError
    try:
